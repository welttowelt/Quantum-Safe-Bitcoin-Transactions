...
% --- Title ---
\title{\textbf{Quantum-Safe Bitcoin Transactions Without Softforks}}
...
\begin{abstract}
We present QSB, a Quantum Safe Bitcoin transaction scheme that requires no changes to the Bitcoin protocol and remains secure even in the presence of Shor's algorithm. Our construction modifies \textsc{Binohash} (Linus, 2026), replacing its non-quantum-safe component---the signature-size-based proof-of-work puzzle---with a hash-to-sig puzzle whose security depends only on the pre-image resistance of RIPEMD-160. The resulting scheme achieves ${\sim}118$-bit second pre-image resistance under the Shor threat model (roughly half under Grover's algorithm), at an estimated off-chain GPU cost of a few hundred dollars. The scheme fits within Bitcoin's legacy script constraints of 201 opcodes and 10{,}000 bytes.
\end{abstract}
...
We further introduce \emph{bonus keys}---subset selections that contribute to the search space without requiring Lamport verification---to bridge the gap between the fixed $2^{46}$ puzzle target and the available opcode budget. The resulting scheme achieves~${\sim}118$-bit pre-image resistance and fits within 201 opcodes, at an off-chain GPU cost of a few hundred dollars.
...
\begin{itemize}\setlength{\itemsep}{1pt}\setlength{\parskip}{0pt}
    \item Both the off-chain computational cost and the on-chain transaction cost do not scale to the full range of users, amounts, and throughput that Bitcoin targets (also, these are non-standard transactions, though with some security tradeoffs we might be able to bring them closer to standard).
    \item Apart from the cost, the user experience for generating the transaction is more complex than standard Bitcoin usage (see Section~\ref{sec:operational-architecture}).
    \item The scheme does not yet cover all Bitcoin use cases (e.g., Lightning Network channels).
\end{itemize}
\noindent We believe improvements can be made on all three fronts, though it would not be the most straightforward path. To the extent that the quantum threat is believed to be real, it remains necessary to continue the ongoing effort to research and implement the best possible solution for Bitcoin---one that is maximally efficient, user-friendly, and answers Bitcoin's needs, through protocol-level changes.
...
\noindent For a full discussion of additional constraints---including stack element size limits, stack depth, and transaction relay considerations---see \textsc{Binohash}~\cite[Section~6]{linus2024binohash}. Furthermore, as noted in \textsc{Binohash}, the script size exceeds the 520-byte limit for P2SH redeem scripts, so the transaction must use a bare script output (scriptPubKey contains the script directly).
...
\begin{itemize}
    ...
    \item SHA-256: $2^{45}$ classically $\to$ ${\sim}2^{22.5}$ with Grover.
\end{itemize}
...
\begin{tikzpicture}[
    ...
    box/.style={draw, rounded corners=3pt, minimum height=0.7cm, minimum width=3.0cm, text centered, thick, align=center, fill=white},
...
% === PINNING ===
\node[font=\bfseries\small] (pin_title) {Transaction Pinning};
\node[pin, below=0.3cm of pin_title] (p1) {Hardcoded signature\\commits to \texttt{SIGHASH\_ALL}};
...
\node[below=0.2cm of p3, font=\scriptsize, text=gray!50!black, align=center] {Any tx modification\\requires new puzzle solve};

% === ROUND (single, representative) ===
\node[font=\bfseries\small, right=4.5cm of pin_title] (r_title) {Digest Round ($\times 2$)};

\node[rnd, below=0.3cm of r_title] (r1) {Spender chooses a subset\\of $t$ from $n$ dummy sigs};
...
\begin{tikzpicture}[
    ...
    every node/.style={font=\footnotesize},
    box/.style={draw, rounded corners=3pt, minimum height=0.65cm, minimum width=3.3cm, text centered, thick, align=center, fill=white},
    secure/.style={box, fill=green!8},
    gpu/.style={box, fill=blue!8},
    data/.style={-{Stealth[length=4pt]}, thick, dashed, blue!60!black},
    arrow/.style={-{Stealth[length=4pt]}, thick},
]

% === LEFT: Secure Device ===
\node[font=\bfseries\small, text=green!40!black] (stitle) {Secure Device};
\node[font=\scriptsize, text=gray, below=0.1cm of stitle] (ssub) {(hardware wallet / air-gapped)};

\node[secure, below=0.2cm of ssub] (s1) {Generate HORS secrets\\and commitments};
\node[secure, below=of s1] (s2) {Build locking script\\(public data only)};
\node[secure, below=of s2] (s3) {Create unsigned tx\\template};
\node[secure, below=2.5cm of s3] (s4) {Verify puzzle solution\\(key recovery + RIPEMD)};
...
% === RIGHT: GPU Farm ===
\node[font=\bfseries\small, text=blue!40!black, right=4.5cm of stitle] (gtitle) {GPU Farm};
\node[font=\scriptsize, text=gray, below=0.1cm of gtitle] (gsub) {(vast.ai / untrusted)};

\node[gpu, below=1.4cm of gsub] (g1) {Receive locking script\\+ unsigned tx};
...
\draw[data] (s3.east) -- +(1.2,0) |- (g1.west) node[pos=0.25, above, font=\scriptsize\itshape, text=blue!50!black] {public data};
\draw[data] (g4.west) -- +(-1.2,0) |- (s4.east) node[pos=0.25, above, font=\scriptsize\itshape, text=blue!50!black] {solution};

% === Annotation ===
\node[draw=red!50, rounded corners=3pt, fill=red!3, font=\scriptsize, text=red!60!black, align=center, below=0.3cm of s6, minimum width=3.3cm] (note) {HORS secrets never\\leave this device};
...
\noindent \textbf{Config~A} ($t = 8{+}1\mathrm{b}, 7{+}2\mathrm{b}$) is the recommended configuration: it fits exactly within the 201-opcode limit and eliminates grinding entirely ($\binom{150}{9} \approx 2^{46.2} \geq 2^{46}$), trading~${\sim}20$ bits of pre-image resistance (from~$2^{138}$ to~$2^{118}$) for dramatically lower off-chain cost---a margin that remains ample for all foreseeable threats. Config~B ($t = 8{+}1\mathrm{b}, 8$) preserves full digest entropy but exceeds the opcode limit by~1.
...
\paragraph{SHA-256 round puzzle variant.}
As noted in Section~\ref{sec:ripemd-puzzle}, using \texttt{OP\_SHA256} instead of \texttt{OP\_RIPEMD160} for the round puzzles yields a slightly higher DER probability (${\sim}2^{-45}$ vs.~${\sim}2^{-46}$). The per-candidate GPU cost is essentially identical, since the final hash is less than~$2\%$ of the total work (dominated by EC scalar multiplication and modular inversion). However, the higher DER probability means each round requires fewer candidates and the pool size~$n$ can be reduced while maintaining $P(\text{round}) \geq 1$. Table~\ref{tab:sha256-variant} compares the options.
...
\toprule
\textbf{Puzzle} & $n$ & \textbf{Script} & $P(\text{rnd})$ & \textbf{Digest} & \textbf{Pre-image} & \textbf{Collision} & \textbf{Cost} \\
\midrule
RIPEMD, $n{=}150$ & 150 & ${\sim}9{,}500$\,B & 1.18 & 84.1\,b & $2^{118}$ & $2^{88}$ & $\$75$--$\$150$ \\
SHA-256, $n{=}150$ & 150 & ${\sim}9{,}500$\,B & 2.36 & 84.1\,b & $2^{115}$ & $2^{87}$ & $\$65$--$\$130$ \\
SHA-256, $n{=}145$ & 145 & ${\sim}9{,}200$\,B & 1.72 & 83.2\,b & $2^{115}$ & $2^{87}$ & $\$50$--$\$100$ \\
SHA-256, $n{=}140$ & 140 & ${\sim}8{,}900$\,B & 1.24 & 82.3\,b & $2^{115}$ & $2^{86}$ & $\$40$--$\$80$ \\
\bottomrule
\end{tabular}
\end{center}
\caption{Comparison of RIPEMD-160 and SHA-256 round puzzles (Config~A: $t_1 = 8{+}1\mathrm{b}$, $t_2 = 7{+}2\mathrm{b}$). The SHA-256 variant trades ${\sim}3$ bits of pre-image resistance for ${\sim}20$--$50\%$ lower off-chain cost, and enables a smaller pool size~$n$ (saving script bytes). A hybrid configuration---RIPEMD-160 for pinning, SHA-256 for rounds---is also possible.}
\label{tab:sha256-variant}
\end{table}
...
\begin{itemize}
    ...
    \item Create a version that is closer to standard or perhaps even fully standard, potentially by lowering the security parameters and/or using a different method to derive the digest.\footnote{We tested using hash chains instead for the digest generation, but the opcode and script size cost exceeded the limit for the security level we were targeting. Perhaps it can be done in a better way.}
...
We thank Robin Linus for reviewing and pointing out that the SHA-256 puzzle probability is lower than previously assumed, leading to the cost--security tradeoff presented in Table~\ref{tab:sha256-variant}, as well as for useful discussions about \textsc{Binohash}. We also thank Eli Ben-Sasson and Adam Borco for comments that helped improve the presentation of this article.
