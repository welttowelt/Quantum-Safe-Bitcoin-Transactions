...
`QSB Studio` keeps each run in its own workspace under `studio/sessions/` and wraps the real `setup → export → export-digest → assemble` flow with artifact inspection, automatic hit ingestion, session cloning, and Vast fleet orchestration/status tracking. See [`studio/README.md`](studio/README.md) for details.
