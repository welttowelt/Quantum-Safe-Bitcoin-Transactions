...
__global__ void kernel_digest(
    ...
    for(int ri=0;ri<2&&!v;ri++){
        ...
        int vv = calibrate_mode ? gpu_is_der_relaxed(h,32) : (easy_mode ? gpu_is_der_easy(h,32) : (gpu_is_valid_der(h,32) && gpu_der_r_on_curve(h)));
        if(vv){
        ...
        vv = calibrate_mode ? gpu_is_der_relaxed(h2,32) : (easy_mode ? gpu_is_der_easy(h2,32) : (gpu_is_valid_der(h2,32) && gpu_der_r_on_curve(h2)));
        if(vv){
...
/* ============================================================
 * DEBUG KERNEL: run full digest pipeline for ONE subset and printf-dump
 * every intermediate value. Mirrors the pin debug kernel.
 * ============================================================ */
__global__ void kernel_debug_digest_one_subset(
    const uint8_t *d_skip,    /* sorted skip indices, t_sel bytes */
    int n_pool, int t_sel,
    const uint32_t *d_midstate,
    const uint8_t *d_prefix_remainder, int prefix_remainder_len,
    const uint8_t *d_dummy_sigs,
    ...
    printf("DBG: prefix_remainder_len = %d\n", prefix_remainder_len);
    printf("DBG: tail_len = %d\n", tail_len);
    printf("DBG: tx_suffix_len = %d\n", tx_suffix_len);

    printf("DBG: skip_indices = ");
    for (int i = 0; i < t_sel; i++) printf("%s%d", i?",":"", d_skip[i]);
    printf("\n");

    /* Build suffix exactly like production kernel */
    uint8_t suffix[8192];
    int pos = 0;
    for (int i = 0; i < prefix_remainder_len; i++) suffix[pos++] = d_prefix_remainder[i];

    int sel = 0;
    for (int i = 0; i < n_pool; i++) {
        if (sel < t_sel && d_skip[sel] == i) { sel++; continue; }
        for (int b = 0; b < SIG_PUSH_SIZE; b++)
            suffix[pos++] = d_dummy_sigs[i * SIG_PUSH_SIZE + b];
    }
    for (int i = 0; i < tail_len; i++) suffix[pos++] = d_tail[i];
    ...
    printf("DBG: suffix_pos = %d\n", pos);
    /* Print first and last 32 bytes for compact diff */
    printf("DBG: suffix_first32 = ");
    for (int i = 0; i < 32 && i < pos; i++) printf("%02x", suffix[i]);
    printf("\n");
    printf("DBG: suffix_last32 = ");
    int start = pos > 32 ? pos - 32 : 0;
    for (int i = start; i < pos; i++) printf("%02x", suffix[i]);
    printf("\n");
    ...
    uint8_t last_block[128];
    int rem = pos - full_blocks * 64;
    for (int i = 0; i < 128; i++) last_block[i] = 0;
    for (int i = 0; i < rem; i++) last_block[i] = suffix[full_blocks*64 + i];
    last_block[rem] = 0x80;
    int nblk = (rem < 56) ? 1 : 2;
    uint64_t bit_len = (uint64_t)total_preimage_len * 8;
    int last = nblk * 64 - 8;
    ...
    uint8_t first[32];
    for (int i = 0; i < 8; i++) {
        first[i*4]=(state[i]>>24)&0xFF; first[i*4+1]=(state[i]>>16)&0xFF;
        first[i*4+2]=(state[i]>>8)&0xFF; first[i*4+3]=state[i]&0xFF;
    }
    DUMP32_BE("first_sha256", first);

    /* Second SHA-256 */
    uint8_t pad[64]; for (int i = 0; i < 64; i++) pad[i] = 0;
    for (int i = 0; i < 32; i++) pad[i] = first[i];
    pad[32] = 0x80; pad[62] = 0x01; pad[63] = 0x00;
    uint32_t b2[16];
    for (int i = 0; i < 16; i++) b2[i] = ((uint32_t)pad[i*4]<<24)|((uint32_t)pad[i*4+1]<<16)|
        ((uint32_t)pad[i*4+2]<<8)|(uint32_t)pad[i*4+3];
    uint32_t s2[8] = {0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,
                      0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19};
    _SHA256Transform(s2, b2);
    uint8_t sighash[32]; for (int i = 0; i < 8; i++) {
        sighash[i*4]=(s2[i]>>24)&0xFF; sighash[i*4+1]=(s2[i]>>16)&0xFF;
        sighash[i*4+2]=(s2[i]>>8)&0xFF; sighash[i*4+3]=s2[i]&0xFF;
    }
    ...
    /* z to scalar */
    uint64_t z[4]; for (int i = 0; i < 4; i++) {
        z[i] = 0; for (int b = 0; b < 8; b++) z[i] |= (uint64_t)sighash[31-i*8-b] << (b*8);
    }
    ...
    uint64_t q2x[4],q2y[4],q2z[5];
    for (int i = 0; i < 4; i++) { q2x[i] = q1x[i]; q2y[i] = q1y[i]; }
    for (int i = 0; i < 5; i++) q2z[i] = q1z[i];
    _PointAddSecp256k1(q2x, q2y, q2z, n2rx, n2ry);
    DUMP_U64x4("q2_proj_x", q2x);
    DUMP_U64x4("q2_proj_y", q2y);
    DUMP_U64x4("q2_proj_z", q2z);

    /* Batch ModInv to affine */
    uint64_t prod[5]={0,0,0,0,0};
    _ModMult(prod, q1z, q2z); _ModInv(prod);
    uint64_t inv1[5], inv2[5];
    _ModMult(inv1, prod, q2z); _ModMult(inv2, prod, q1z);
    ...
    /* For each recid: pubkey + SHA256 + DER check */
    uint64_t *pts_x[2] = {q1x, q2x};
    uint64_t *pts_y[2] = {q1y, q2y};
    for (int ri = 0; ri < 2; ri++) {
        ...
        uint32_t hs[8]; _SHA256Initialize(hs); _SHA256Transform(hs, pb);
        uint8_t h[32]; for (int i = 0; i < 8; i++) {
            h[i*4]=(hs[i]>>24)&0xFF;h[i*4+1]=(hs[i]>>16)&0xFF;
            h[i*4+2]=(hs[i]>>8)&0xFF;h[i*4+3]=hs[i]&0xFF;
        }
...
int main(int argc, char **argv) {
     ...
     */
    {
        ...
        if (debug_idx >= 0 && debug_idx + 1 < argc) {
            const char *subset_str = argv[debug_idx + 1];
            uint8_t skip[32]; int n_skip = 0;
            char buf[256]; strncpy(buf, subset_str, 255); buf[255] = 0;
            char *tok = strtok(buf, ",");
            while (tok && n_skip < t_sel) {
                skip[n_skip++] = (uint8_t)atoi(tok);
                tok = strtok(NULL, ",");
            }
            if (n_skip != t_sel) {
                printf("ERROR: expected %d indices in debug subset, got %d\n", t_sel, n_skip);
                return 1;
            }
            uint8_t *d_skip; cudaMalloc(&d_skip, t_sel);
            cudaMemcpy(d_skip, skip, t_sel, cudaMemcpyHostToDevice);
            printf("\n=== DEBUG MODE: dumping all intermediates for subset = ");
            for (int i = 0; i < t_sel; i++) printf("%s%d", i?",":"", skip[i]);
            printf(" ===\n");
            kernel_debug_digest_one_subset<<<1, 32>>>(
                d_skip, n_pool, t_sel,
                d_mid, d_prem, dp.prefix_remainder_len,
                d_dsigs, d_tail, dp.tail_section_len,
                d_suf, dp.tx_suffix_len,
                dp.total_preimage_len,
            ...
            if (err != cudaSuccess) {
                printf("kernel_debug_digest failed: %s\n", cudaGetErrorString(err));
            }
    ...
    while (!found) {
        ...
        while (!exhausted && !found) {
            ...
            if (h_hit > 0) {
                ...
                /* In calibrate mode, append to accumulate many hits over a 30-min run.
                 * In normal mode, overwrite (we only need the first hit). */
                FILE *ff = fopen(fname, calibrate ? "a" : "w");
                if (ff) {
