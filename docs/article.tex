...
% --- Title ---
\title{\textbf{Quantum-Safe Bitcoin Transactions Today}\\[0.3em]\textbf{Using Quantum Safe Binohash (QSB)}}
...
\begin{abstract}
We present a scheme for quantum-safe Bitcoin transactions that requires no changes to the Bitcoin protocol and remains secure even in the presence of Shor's algorithm. Our construction modifies \textsc{Binohash} (Linus, 2025) to replace the signature-size-based proof-of-work puzzle---which relies on ECDSA hardness---with a hash-based puzzle whose security depends only on the pre-image resistance of RIPEMD-160. The resulting scheme, Quantum Safe Binohash (QSB), achieves ${\sim}117$-bit second pre-image resistance at an off-chain cost of approximately~\$40 under the Shor threat model, and roughly half the security bits under Grover's algorithm. A security--cost tradeoff exists: ${\sim}130$-bit pre-image resistance is achievable at a higher off-chain cost of approximately~\$1{,}000. Both configurations fit within Bitcoin's legacy script constraints of 201 opcodes and 10{,}000 bytes.
\end{abstract}
...
Two recent works bring us close to the goal of quantum-safe Bitcoin transactions. The first is Heilman's ``Signing a Bitcoin Transaction with Lamport Signatures (no OP\_CAT)''~\cite{heilman2024}, followed by the work of Robin Linus---\textsc{Binohash}~\cite{linus2024binohash}---on which the present scheme is based almost entirely.
...
The puzzle signature is verified inside \texttt{CHECKMULTISIG}, which means FindAndDelete modifies the scriptCode---and therefore the sighash---depending on which dummy signatures are included. This gives us $\binom{n}{t}$ distinct sighash candidates from a single fixed transaction, each corresponding to a different subset of dummy signatures. The purpose of the selection stage is to translate the digest provided in the witness into the specific subset that solves the puzzle, after verifying the Lamport signature on it.
...
$$11 t + 8$$
where $t$ is the number of selections per round. The breakdown is: $9t$ for the selection loop (9 opcodes per selection) and $2t + 8$ for the puzzle derivation, \texttt{CHECKSIGVERIFY}, and \texttt{CHECKMULTISIG} setup. For two rounds with $t_1$ and $t_2$ selections, the total is:
...
[TODO: Bonus keys, comparison table, off-chain cost]
...
\bibitem{colliderscript}
    E.~Heilman, V.~Kolobov, A.~Levy, and A.~Poelstra,
    ``ColliderScript: Covenants in Bitcoin via 160-bit Hash Collisions,''
    Cryptology ePrint Archive, Report 2024/1802, 2024.
...
\begin{tikzpicture}[
    ...
    every node/.style={font=\small},
    box/.style={draw, rounded corners=3pt, minimum height=0.8cm, minimum width=3.2cm, text centered, thick, align=center},
    decision/.style={draw, diamond, aspect=2.2, minimum width=2.5cm, text centered, thick, align=center, font=\small},
    arrow/.style={-{Stealth[length=5pt]}, thick},
    label/.style={font=\footnotesize\itshape, text=gray!70!black}
]

% === LEFT: Off-chain Spending Procedure ===
\node[font=\bfseries] (title1) {Off-Chain Spending};

\node[box, fill=gray!10, below=0.5cm of title1] (tx) {Construct tx};
...
\draw[arrow] (powok.east) -- +(0.8,0) node[above, label] {no} |- (subset.east);
\draw[arrow] (digest) -- (both);
\draw[arrow] (both) -- node[right, label] {yes} (done);
\draw[arrow] (both.west) -- +(-1.8,0) node[above, label] {no (next round)} |- (subset.west);
\draw[arrow, dashed] (powok.west) -- +(-2.2,0) |- (tx.west) node[pos=0.25, above, label] {all subsets exhausted};

% === RIGHT: Script Verification ===
\node[font=\bfseries, right=5.5cm of title1] (title2) {Script Verification};

\node[box, fill=orange!10, below=0.5cm of title2] (s1) {Verify 4 pinning\\puzzles (\texttt{OP\_SIZE})};
\node[box, fill=orange!10, below=0.8cm of s1] (s2a) {Check SINGLE\\flag excluded};
\node[box, fill=teal!8, below=0.8cm of s2a, minimum width=3.5cm] (s2b) {Round $r$ ($\times 2$):};
\node[box, fill=teal!8, below=0.6cm of s2b] (s3) {Verify HORS\\preimages};
\node[box, fill=teal!8, below=0.6cm of s3] (s4) {Select $t$ dummy sigs\\(\texttt{OP\_ROLL})};
\node[box, fill=teal!8, below=0.6cm of s4] (s5) {\texttt{CHECKMULTISIG}\\(FindAndDelete)};
\node[box, fill=teal!8, below=0.6cm of s5] (s6) {Check puzzle sig\\size (\texttt{OP\_SIZE})};
\node[box, fill=green!15, below=0.8cm of s6] (sok) {Script succeeds};
...
\draw[decorate, decoration={brace, amplitude=6pt, mirror}, thick, gray]
    ([xshift=1.9cm]s3.north east) -- ([xshift=1.9cm]s6.south east)
    node[midway, right=8pt, font=\footnotesize, gray] {$\times 2$ rounds};
