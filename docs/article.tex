...
\author{Avihu Mordechai Levy\\\texttt{@avihu28}\\\texttt{avihu@starkware.co}}
\date{\today}
...
\begin{abstract}
We present the first quantum-safe Bitcoin transaction scheme that requires no changes to the Bitcoin protocol and remains secure even in the presence of Shor's algorithm. Our construction modifies \textsc{Binohash} (Linus, 2026), replacing its non-quantum-safe component---the signature-size-based proof-of-work puzzle---with a hash-based puzzle whose security depends only on the pre-image resistance of RIPEMD-160. The resulting scheme, Quantum Safe Bitcoin (QSB), achieves ${\sim}118$-bit second pre-image resistance under the Shor threat model (roughly half under Grover's algorithm), at an estimated off-chain GPU cost of a few hundred dollars. The scheme fits within Bitcoin's legacy script constraints of 201 opcodes and 10{,}000 bytes.
\end{abstract}
...
In this work, we modify the \textsc{Binohash} scheme to address both issues identified in Section~\ref{sec:issues}. We replace the signature-size-based PoW puzzle with a hash-based puzzle that does not depend on the ECDSA $r$ value or on any elliptic curve hardness assumption. Furthermore, since the nonce in our new puzzle is a public key---derived via ECDSA key recovery from a hardcoded signature---we are free to hardcode the appropriate sighash flag (\texttt{SIGHASH\_ALL}) directly in the locking script, eliminating the sighash flag uncertainty.
...
We now present our modifications to \textsc{Binohash}. The central change is replacing the signature size puzzle (Section~\ref{sec:sig-size-puzzle}) with a \emph{hash-based} puzzle whose security relies only on the pre-image resistance of cryptographic hash functions---not on any elliptic curve hardness assumption. This directly addresses the quantum vulnerability identified in Section~\ref{sec:quantum-vulnerability}.

\subsection{Hash-to-Signature Puzzle}
...
A DER-encoded ECDSA signature has a rigid structure: \texttt{30 [len] 02 [r-len] [r] 02 [s-len] [s] [sighash]}, where the length fields must be internally consistent, both~$r$ and~$s$ must be positive integers with no unnecessary leading zeros (i.e., MSB~$< 128$), and the sighash byte must be a valid flag. A random byte string satisfying all of these constraints simultaneously is a rare event---but not impossibly rare.
...
Linus~\cite{linus2024sha2ecdsa} first described this approach using SHA-256. A random 32-byte string is a valid DER-encoded ECDSA signature with probability approximately~$2^{-51}$. The DER structure admits multiple valid combinations of~$r$ and~$s$ lengths within 32 bytes; the dominant constraint is that both~$r$ and~$s$ must have MSB~$< 128$ (each contributing roughly 1 bit of rejection), combined with the requirement that the length fields are internally consistent.
...
As noted in \textsc{Binohash} Appendix~D~\cite{linus2024binohash}, the same approach works with RIPEMD-160, producing 20-byte outputs. A random 20-byte string is a valid DER-encoded ECDSA signature with probability approximately~$2^{-46}$.

The higher probability arises because 20-byte DER signatures, while shorter, admit a wider range of valid $(r\text{-len}, s\text{-len})$ combinations relative to their total length. The dominant constraints remain the same: the \texttt{30} tag byte, consistent length fields, positive $r$ and~$s$ values with MSB~$< 128$, and a valid sighash byte. See~\cite[Appendix~D]{linus2024binohash} for a detailed enumeration of the valid DER formats at 20 bytes.
...
\subsection{Using the Hash-to-Signature Puzzle in Script}
\label{sec:puzzle-in-script}

We now describe how the hash-to-signature puzzle and ECDSA key recovery are combined to create a quantum-safe PoW check that is fully verifiable in Bitcoin Script.
...
An important distinction between the hash-to-signature puzzle and the signature size puzzle is that the hash-to-signature puzzle has a \emph{fixed} difficulty: it is determined entirely by the hash function and the target primitive (DER-encoded ECDSA signature). For RIPEMD-160, this is~${\sim}2^{46}$; for SHA-256, ${\sim}2^{51}$. There is no knob to tune.
...
We now describe the complete construction, replacing the signature size puzzle in \textsc{Binohash} with the hash-to-signature puzzle throughout.

\subsubsection{Transaction Pinning}
\label{sec:qs-pinning}

Transaction pinning ensures that modifying any part of the spending transaction requires solving the hash-to-signature puzzle anew (${\sim}2^{46}$ work). In \textsc{Binohash}, this requires four signature size puzzles at a cost of 13 opcodes. In our scheme, a single application of the hash-to-signature puzzle suffices, since the hardcoded \texttt{SIGHASH\_ALL} flag commits to all transaction fields.
...
\begin{itemize}
    ...
    \item \textbf{Step 3} (hash-to-signature check): \texttt{OP\_RIPEMD160} hashes \texttt{key\_nonce} to produce \texttt{sig\_puzzle}. Then \texttt{OP\_SWAP} brings \texttt{key\_puzzle} on top, and the second \texttt{OP\_CHECKSIGVERIFY} verifies that \texttt{sig\_puzzle} is a valid DER signature.
\end{itemize}
...
QSB Config~A ($t{=}8{+}1\mathrm{b}, 7{+}2\mathrm{b}$) & $2^{47.7}$ & ${\sim}\$200$--$\$500$ \\
\bottomrule
\end{tabular}
\end{center}
...
We have presented Quantum Safe Bitcoin (QSB), a modification of \textsc{Binohash}~\cite{linus2024binohash} that eliminates the scheme's reliance on Shor-breakable assumptions. The key change is replacing the signature-size puzzle---which depends on the hardness of finding small elliptic curve $r$-coordinates---with a hash-to-signature puzzle whose security rests entirely on the pre-image resistance of RIPEMD-160. This enables signing Bitcoin transactions in a quantum-safe manner today, without any changes to the Bitcoin protocol: QSB operates under the existing consensus rules for legacy transactions. To the best of our knowledge, this is the first scheme to achieve quantum-safe Bitcoin transactions on the live network.
...
\begin{enumerate}
    \item \textbf{Hash-to-signature puzzle.} Replacing the OP\_SIZE-based signature puzzle with a RIPEMD-160 hash-to-signature puzzle, achieving quantum safety. As a side effect, the hardcoded \texttt{SIGHASH\_ALL} flag in our puzzle resolves the sighash flag uncertainty identified in \textsc{Binohash}.
...
Future work may include optimizations to off-chain search cost (Appendix~\ref{app:future-optimizations}), improved opcode utilization to increase security margins, and exploration of other applications of the hash-to-signature puzzle beyond transaction pinning.
...
\section{Binohash Scheme Diagrams}
\label{app:binohash-diagrams}
...
\begin{tikzpicture}[
    ...
    box/.style={draw, rounded corners=2pt, minimum height=0.6cm, minimum width=2.6cm, text centered, thick, align=center},
    decision/.style={draw, diamond, aspect=2.2, minimum width=2cm, text centered, thick, align=center, font=\footnotesize},
    arrow/.style={-{Stealth[length=4pt]}, thick},
    label/.style={font=\scriptsize\itshape, text=gray!70!black}
]

% === LEFT: Off-chain Spending Procedure ===
\node[font=\bfseries\footnotesize] (title1) {Off-Chain Spending};

\node[box, fill=gray!10, below=0.3cm of title1] (tx) {Construct tx};
\node[box, fill=blue!8, below=of tx] (pin) {Grind tx params\\(solve pinning)};
\node[box, fill=blue!8, below=of pin] (subset) {Try next subset\\$\binom{n}{t}$};
\node[decision, fill=red!5, below=of subset] (powok) {PoW\\solved?};
\node[box, fill=green!10, below=of powok] (digest) {Record digest\\+ HORS preimages};
\node[decision, fill=red!5, below=of digest] (both) {Both\\rounds?};
\node[box, fill=green!15, below=of both] (done) {Broadcast tx};

\draw[arrow] (tx) -- (pin);
\draw[arrow] (pin) -- node[right, label] {tx fixed} (subset);
\draw[arrow] (subset) -- (powok);
\draw[arrow] (powok) -- node[right, label] {yes} (digest);
\draw[arrow] (powok.east) -- +(0.7,0) node[above, label] {no} |- (subset.east);
\draw[arrow] (digest) -- (both);
\draw[arrow] (both) -- node[right, label] {yes} (done);
\draw[arrow] (both.west) -- +(-1.5,0) node[above, label] {no (next round)} |- (subset.west);
\draw[arrow, dashed] (powok.west) -- +(-1.8,0) |- (tx.west) node[pos=0.25, above, label] {all subsets exhausted};

% === RIGHT: Script Verification ===
\node[font=\bfseries\footnotesize, right=4.5cm of title1] (title2) {Script Verification};

\node[box, fill=orange!10, below=0.3cm of title2] (s1) {Verify 4 pinning\\puzzles (\texttt{OP\_SIZE})};
\node[box, fill=orange!10, below=0.5cm of s1] (s2a) {Check SINGLE\\flag excluded};
\node[box, fill=teal!8, below=0.5cm of s2a, minimum width=2.8cm] (s2b) {Round $r$ ($\times 2$):};
\node[box, fill=teal!8, below=0.4cm of s2b] (s3) {Verify HORS preimages};
\node[box, fill=teal!8, below=0.4cm of s3] (s4) {Select $t$ dummy sigs};
\node[box, fill=teal!8, below=0.4cm of s4] (s5) {\texttt{CHECKMULTISIG}\\(FindAndDelete)};
\node[box, fill=teal!8, below=0.4cm of s5] (s6) {Check sig size\\(\texttt{OP\_SIZE})};
\node[box, fill=green!15, below=0.5cm of s6] (sok) {Script succeeds};
...
\draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick, gray]
    ([xshift=1.6cm]s3.north east) -- ([xshift=1.6cm]s6.south east)
    node[midway, right=6pt, font=\scriptsize, gray] {$\times 2$ rounds};

\end{tikzpicture}
\caption{The \textsc{Binohash} construction. \textbf{Left:} the off-chain spending procedure. \textbf{Right:} the on-chain script verification.}
\label{fig:binohash-scheme}
\end{figure}
...
\label{app:future-optimizations}

Several directions may reduce the off-chain cost:

\begin{itemize}
    \item \textbf{Incremental sighash.} As analyzed in Section~\ref{sec:offchain-cost}, precomputing SHA-256 states over the fixed portions of the scriptCode can reduce the per-candidate sighash cost by up to~$5\times$ for Round~2. A GPU implementation exploiting this could significantly narrow the cost gap with \textsc{Binohash}.

    \item \textbf{Precomputed EC tables.} The ECDSA key recovery step---a scalar multiplication by the generator~$G$---can be accelerated using precomputed window tables. An 8-bit window table (512\,KB, fits in GPU L2 cache) reduces the number of EC operations from~${\sim}384$ to~${\sim}64$ per recovery, a~$6\times$ speedup on the dominant per-candidate cost.

    \item \textbf{Ordered subset enumeration.} Enumerating subsets in an order where consecutive subsets differ by a single element (revolving-door combinations) enables incremental scriptCode updates, avoiding full FindAndDelete rescans.

    \item \textbf{Round ordering.} Searching Round~2 before Round~1 is strictly cheaper, since Round~1's data forms a precomputable prefix for Round~2's sighash (Section~\ref{sec:offchain-cost}). Failed Round~2 searches avoid the more expensive Round~1 entirely.

    \item \textbf{Alternative hash functions.} If a future soft fork introduces \texttt{OP\_SHA256} with a lower hash-to-signature probability (e.g., ${\sim}2^{51}$ for 32-byte outputs), the puzzle target could be adjusted. However, this would increase the per-candidate work proportionally.
\end{itemize}
