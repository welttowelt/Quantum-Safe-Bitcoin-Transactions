 ...
 *   [4: suffix_template_len]
 *   [4: sequence_offset]
 *   [4: locktime_offset]
 *   [32: midstate (8 × uint32 big-endian)]
 *   [suffix_template_len: suffix template]
 *   [32: neg_r_inv LE]
 *   [32: u2r_x LE]
 *   [32: u2r_y LE]
 *
...
typedef struct {
    ...
    uint32_t locktime_offset;
    uint32_t midstate[8];
    uint8_t *suffix_template;
...
static int load_pinning_params(const char *filename, pinning_params_t *p) {
    ...
    if (fread(&p->suffix_template_len, 4, 1, f) != 1) goto err;
    if (fread(&p->sequence_offset, 4, 1, f) != 1) goto err;
    if (fread(&p->locktime_offset, 4, 1, f) != 1) goto err;
    ...
    p->suffix_template = (uint8_t *)malloc(p->suffix_template_len);
    if (fread(p->suffix_template, 1, p->suffix_template_len, f) != p->suffix_template_len) goto err;
    if (fread(p->neg_r_inv, 1, 32, f) != 32) goto err;
    if (fread(p->u2r_x, 1, 32, f) != 32) goto err;
    if (fread(p->u2r_y, 1, 32, f) != 32) goto err;
    
    fclose(f);
    printf("  Loaded pinning params: preimage=%u, suffix=%u, seq@%u, lt@%u\n",
           p->total_preimage_len, p->suffix_template_len, p->sequence_offset, p->locktime_offset);
    return 0;
